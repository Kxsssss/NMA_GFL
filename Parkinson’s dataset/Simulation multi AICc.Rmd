...
for (i in 1:length(dat_list)){
  mse_full = sqrt(apply(dat_list[[i]][22:31], 2, sum)/1000)
  mse_best = sqrt(apply(dat_list[[i]][32:41], 2, sum)/1000)
  tit = dat_titles[i]
  ...
  if(i == 1){
    plot(mse_full, mse_best, xlab = "MSE of Full Model", ylab = "MSE of Best Model",
         xlim=c(0, sqrt(0.004)), 
         ylim=c(0, sqrt(0.004)))
    text(mse_full[1:4], mse_best[1:4]-0.0002, labels=c("1-2","1-3", "1-4", "1-5"))
  }else if(i==2){
    plot(mse_full, mse_best, xlab = "MSE of Full Model", ylab = "MSE of Best Model",
         xlim=c(0,sqrt(0.75)), 
         ylim=c(0, sqrt(0.75)))
    text(mse_full[1:4], mse_best[1:4]-0.05, labels=c("1-2","1-3", "1-4", "1-5"))
  }else{
    plot(mse_full, mse_best, xlab = "MSE of Full Model", ylab = "MSE of Best Model",
         xlim=c(0, 6.5), 
         ylim=c(0, 6.5))
    text(mse_full[1:4], mse_best[1:4]-1, labels=c("1-2","1-3", "1-4", "1-5"))
  }
  title(paste(tit))
  abline(0,1, col = "red")
}
...
for (i in 1:length(dat_list)){
  ...
  if(i<=4){
    plot(mse_full, mse_best, xlab = "RMSE of Full Model", ylab = "RMSE of Best Model",
         xlim=c(0, sqrt(0.004)), 
         ylim=c(0, sqrt(0.004)))
    text(mse_full[1:4], mse_best[1:4]-0.0002, labels=c("1-2","1-3", "1-4", "1-5"))
  }else if(i<=8){
...
for (i in 1:length(dat_list)){
  ...
  mse_f = data.frame(matrix(round(mse_full, 4), ncol = 1))
  colnames(mse_f) = tit
  mse_f$Colour="gray45"
  mse_f$Colour[1:4]="black"
  if(i<=4){
    plot(mse_full, mse_best, xlab = "RMSE of Full Model", ylab = "RMSE of Best Model",
         xlim=c(0, sqrt(0.004)), 
         ylim=c(0, sqrt(0.004)),
...
```{r}
mse_full = sim_sum1_1a[,42]
mse_best = sim_sum1_1a[,43]
mse_full1 = sim_sum1_1a[,25]
mse_best1 = sim_sum1_1a[,35]
par(mfrow=c(2,2))
boxplot(mse_full, main = "Full model - estimated 1-5")
boxplot(mse_best, main = "Best model - estimated 1-5")
boxplot(mse_full1, main = "Full model - difference^2 1-5")
boxplot(mse_best1, main = "Best model - difference^2 1-5")
```
```{r}
hist(mse_best)
hist(mse_full)
